...
router.post("/postPic/:id", auth, upload.single("file"), async (req, res) => {
  try {
    ...
    let x = await Post.findByIdAndUpdate(req.params.id, {
      $push: { images: img },
    });
    res.status(200).json({ file: req.file });
  } catch (err) {
...
router.delete("/deletePostPic/:id&:postid", auth, async (req, res) => {
  try {
    ...
    let newpost = post.images.filter((id) => {
      return id.imageId.toString() !== req.params.id.toString();
    });
